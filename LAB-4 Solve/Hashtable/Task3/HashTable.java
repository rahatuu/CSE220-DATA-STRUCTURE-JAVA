...
public class HashTable {
 ...
 //you need to COMPLETE this method
    ...
    public void remove( Integer key ){
      ...
      if (this.ht[hashedIndex].key==key){
           PairNode garbage= this.ht[hashedIndex];
           this.ht[hashedIndex]=this.ht[hashedIndex].next;
           garbage.key=null;garbage.value=null;garbage=null;}
      else{
        PairNode current=this.ht[hashedIndex], prev=null;
        for(int i=0;current!=null;i++){
          if(current.key==key){ 
            PairNode gab=current;
            prev.next=current.next;
            gab.key=null;gab.value=null;gab=null;}
        prev = current;
