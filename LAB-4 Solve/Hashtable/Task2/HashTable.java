...
public class HashTable {
    ...
    private int hashFunction( String key ){
      ...
      if(key.length()%2!=0){
        for(int i=1;i<key.length();i+=2){
          asciiconcat+= (int)key.charAt(i);}}
      else{for(int i=0;i<key.length();i+=2){
          asciiconcat+= (int)key.charAt(i);}}
 ...
 //then inserts it in the proper hashed index
    ...
    public void insert(String key, Integer value){
            ...
            FruitNode newPNode = new FruitNode( key, value );
            if ( this.ht[hashedIndex] == null ){
              this.ht[hashedIndex] = newPNode;}
            else {
              if((int)this.ht[hashedIndex].fruit[1]<(int)newPNode.fruit[1]){
                newPNode.next = this.ht[hashedIndex];
                this.ht[hashedIndex] = newPNode;}
              else{FruitNode current = this.ht[hashedIndex];
