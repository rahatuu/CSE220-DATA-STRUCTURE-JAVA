...
public class HashTable {
 ...
 //you need to COMPLETE this method
    private int hashFunction( String key ){
      int asciiconcat=0;
      if(key.length()%2!=0){key=key+"N";}
      for(int i=0;i<key.length();i+=2){
          char ch1= key.charAt(i); char ch2=key.charAt(i+1);
          asciiconcat+= Integer.parseInt((Integer.toString((int)ch1)+(Integer.toString((int)ch2))));}  
        return asciiconcat%ht.length; 
    }
 ...
 //you need to COMPLETE this method
    ...
    public String searchHashtable( Object[] keyValuePair ){
      ...
      PairNode present=ht[index];
      if(ht[index]==null){return "Not found";}
      else{
        for(int i=0;present!=null;i++){
          if(present.key.equals(keyValuePair[0])){return "Found";}present=present.next;}
        return "Not Found";}}
    }
